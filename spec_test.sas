...
%MACRO spec_test(libnm = , spec_path = , spec_name = );
  ...
  PROC IMPORT DATAFILE ="&spec_path.\&spec_name..xlsx" OUT = spec DBMS = xlsx REPLACE ;
	  SHEET = 'Variables';
	RUN;


	DATA vcol ( KEEP   = libname memname name len label format  lib_data var
	            RENAME = (memname = dataset name = variable )
              WHERE  = ( UPCASE(STRIP(libname)) = %UPCASE("&libnm.")  ));
	...
	DATA spec_f ( KEEP   = dataset variable label length  format
                RENAME = (label = spec_label length = spec_length   format = spec_format ));

    LENGTH dataset variable $ 50 format $ 20;
	  SET spec;

		IF STRIP(format) ^= "" AND SUBSTR( STRIP(format), LENGTH(STRIP(format)) ) ^= "." THEN format = STRIP(format)||".";
	RUN;
    ...
    IF STRIP(spec_label) ^= STRIP(label) THEN DO;
		  ...
		  PUT "Mismatch in LABEL."  @22 dataset=    @40 var =  @55 label =  100 spec_label = ;
        ...
        missname = STRIP(missname) || " Length";
		  ...
		  PUT "Mismatch in LENGTH." @22 dataset=   @40 var =  @55 len =  @90 spec_length =  ;
    ...
    ELSE IF STRIP(spec_length) ^= STRIP(len) THEN DO;
		...
		IF STRIP(spec_format) = "ISO 8601." THEN DO;
		  ...
		  IF STRIP(spec_length) ^= "" AND STRIP(spec_length) ^= STRIP(len) THEN startus ="ISO length matched";
    ...
    ELSE IF STRIP(spec_format) ^= STRIP(format) THEN DO;
		  ...
		  PUT "Mismatch in LENGTH." @22 dataset=   @40 var =  @55 format =  @90 spec_format =  ;
...
%spec_test( libnm     = sdtmf  ,
            spec_path = !Clinrep\Agilis-SA\21LAQ01\Pgm\amerged\For_Supportive_Programmer\Numan   ,
