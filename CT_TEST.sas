PROC IMPORT DATAFILE
	="\\Path of the file\SDTM Terminology.xlsx"
	OUT=ct DBMS=xlsx REPLACE ;
	SHEET='SDTM Terminology 2022-09-30';
RUN;

PROC IMPORT DATAFILE
	="\\Path of the file\Numan\define-2022-10-10T10-29-30-976_v8.xlsx"
	OUT=define_code DBMS=xlsx REPLACE ;
	SHEET='Codelists';
RUN;
...
DATA ct_test1 (KEEP=code codelist_code codelist_name cdisc_submission_value
	extensible );
...
/*RUN;*/
PROC SQL;
	CREATE TABLE check1 AS SELECT a.id, a.name, a.nci_codelist_code,
		a.nci_term_code, a.term, b.cdisc_submission_value, b.codelist_name,
		b.extensible, b.codelist_code, b.code FROM define_srt AS a LEFT JOIN
		ct_srt as b ON STRIP(a.codelist_code)=STRIP(b.codelist_code) AND
		STRIP(a.code)=STRIP(b.code) ORDER BY b.codelist_code, b.code;
QUIT;
