...
%MACRO c_attrib(dataset = , path = , xlsx_file = , from = , del =  );
  ...
  %******************************************************************;
	...
	PROC IMPORT DATAFILE ="&path.\&xlsx_file."  OUT = spec_datasets DBMS = xlsx REPLACE ;
	  SHEET = 'Datasets';
	RUN;
  ...
  %******************************************************************;
	...
	%IF &var_end_fl. = 0 %THEN %DO;
	  %PUT WARNING: Dataset %UPCASE(&dataset.) is not found in the "Variable" sheet in XLSX specification file.;
    %ABORT CANCEL;
	%END;
  ...
  DATA _NULL_;
	  SET spec_var;
		...
		IF _N_ = 1 THEN DO;
		  ...
		  IF VTYPE(significant_digits)      = "N" THEN CALL SYMPUTX("decimal_num", 1);
			ELSE IF VTYPE(significant_digits) = "C" THEN CALL SYMPUTX("decimal_num", 0);
	  ...
	  DATA specvartype_chng ( RENAME = ( new_fmt = format ) );
		  ...
		  SET spec_var ( RENAME = (format = old_fmt) );
			...
			IF STRIP(UPCASE(data_type)) IN("TEXT", "DATETIME", "CHAR") THEN DO;
			  ...
			  IF STRIP(new_fmt) ^= "" AND SUBSTR(STRIP(new_fmt), 1,1) ^= "$" THEN new_fmt = "$"||STRIP(new_fmt);
  ...
  %ELSE %IF &length_num. = 0 AND &format_num. = 1 AND &decimal_num. = 1 %THEN %DO;
	  ...
	  DATA specvartype_chng ( RENAME = ( new_fmt = format  new_decimal = significant_digits ) );
		  LENGTH new_fmt $ 50 new_decimal $ 10;
		  SET spec_var ( RENAME = ( format = old_fmt  significant_digits = old_decimal ) );
  ...
  %ELSE %IF &length_num. = 1 AND &format_num. = 0 AND &decimal_num. = 1 %THEN %DO;
	  ...
	  DATA specvartype_chng ( RENAME = ( new_len = length  new_decimal = significant_digits ) );
		  LENGTH new_len $ 10 new_decimal $ 10;
		  SET spec_var ( RENAME = ( length = old_len   significant_digits = old_decimal ) );
  ...
  %ELSE %IF &length_num. = 1 AND &format_num. = 1 AND &decimal_num. = 1 %THEN %DO;
	  ...
	  DATA specvartype_chng ( RENAME = ( new_len = length  new_fmt = format  new_decimal = significant_digits ) );
		  LENGTH new_len $ 10 new_fmt $ 50 new_decimal $ 10;
		  SET spec_var ( RENAME = ( length = old_len  format = old_fmt  significant_digits = old_decimal ) );
  ...
  %******************************************************************;
    ...
    %IF &change_type. = 1 %THEN %DO;
		  ...
		  SET specvartype_chng ( KEEP   = order dataset variable data_type label length format significant_digits
                             RENAME = (format = fmt length = len ) 
                             WHERE  = ( UPCASE(STRIP(dataset)) = UPCASE("&dataset.")) ) ;
		  
		%END;
		%ELSE %IF &change_type. = 0 %THEN %DO;

		  SET spec_var( KEEP   = order dataset variable data_type label length format significant_digits
                    RENAME = (format = fmt length = len ) 
                    WHERE  = ( UPCASE(STRIP(dataset)) = UPCASE("&dataset.")) ) ;
		%END;
    ...
    IF UPCASE(STRIP(data_type)) = "FLOAT" AND STRIP(fmt) = "" AND STRIP(len) ^= "" AND STRIP(significant_digits) ^= "" THEN fmt = STRIP(len)|| "."|| STRIP(significant_digits);

	  IF STRIP(fmt) ^= "" THEN DO;

			IF FIND(STRIP(fmt), ".") = 0 THEN DO;
		    IF SUBSTR(STRIP(fmt), LENGTHN(STRIP(fmt)) ) ^= "." THEN fmt = STRIP(fmt)||".";
			END;
	...
	*Maximum length;
  PROC SQL NOPRINT;
	  SELECT  MAX(varlen) AS mvarlen
		      , MAX(lenlen) AS mlenlen
					, MAX(fmtlen) AS mfmtlen
					, MAX(lablen) AS mlablen
		INTO   : mvlen
	       , : mlen
	       , : mflen 
	       , : mlblen
  ...
  %******************************************************************;
	...
	DATA _NULL_;
	  SET formatting END= eof;
		...
		IF _N_ = 1 THEN PUT @3 "CREATE TABLE &dataset. " %IF %LENGTH(&datalabel.) %THEN "(LABEL = ""&datalabel."") " ; "AS" ;
  ...
  %******************************************************************;
		...
		%ELSE %IF &mlen. ^= 0 AND &mflen. ^= 0 AND &mlblen. = 0 %THEN %DO;
          ...
          PUT @3 "SELECT" @12 variable @%EVAL(14+&mvlen.) "LENGTH = " 
				...
				END;
		    ELSE IF STRIP(len) = "" AND STRIP(fmt) ^= "" THEN DO;
          PUT @3 "SELECT" @12 variable @%EVAL(26+&mvlen.+&mlen.) "FROMAT = "
                                       @%EVAL(35+&mvlen.+&mlen.) fmt;

				END;
		    ELSE PUT @3 "SELECT" @12 variable @%EVAL(14+&mvlen.) "LENGTH = " 
                                          @%EVAL(23+&mvlen.) len 
			                                    @%EVAL(26+&mvlen.+&mlen.) "FROMAT = "
                                          @%EVAL(35+&mvlen.+&mlen.) fmt;            
          ...
          PUT @9 "," @12 variable @%EVAL(14+&mvlen.) "LENGTH = " 
				...
				END;
		    ELSE IF STRIP(len) = "" AND STRIP(fmt) ^= "" THEN DO;
          PUT @9 "," @12 variable  @%EVAL(26+&mvlen.+&mlen.) "FROMAT = "
                                   @%EVAL(35+&mvlen.+&mlen.) fmt;
  ...
  %******************************************************************;
		...
		%ELSE %IF &mlen. = 0 AND &mflen. ^= 0 AND &mlblen. ^= 0 %THEN %DO; 
          ...
          PUT @3 "SELECT" @12 variable @%EVAL(14+&mvlen.) "FORMAT = " 
				...
				END;
		    ELSE IF STRIP(fmt) = "" AND STRIP(label) ^= "" THEN DO;
          PUT @3 "SELECT" @12 variable @%EVAL(25+&mvlen.+&mflen.) "LABEL = "
                                       @%EVAL(33+&mvlen.+&mflen.) label;

				END;
		    ELSE PUT @3 "SELECT" @12 variable @%EVAL(14+&mvlen.) "FORMAT = " 
                                          @%EVAL(23+&mvlen.) fmt 
			                                    @%EVAL(25+&mvlen.+&mflen.) "LABEL = "
                                          @%EVAL(33+&mvlen.+&mflen.) label;            
          ...
          PUT @9 "," @12 variable @%EVAL(14+&mvlen.) "FORMAT = " 
				...
				END;
		    ELSE IF STRIP(fmt) = "" AND STRIP(label) ^= "" THEN DO;
          PUT @9 "," @12 variable @%EVAL(25+&mvlen.+&mflen.) "LABEL = "
                                  @%EVAL(33+&mvlen.+&mflen.) label;
  ...
  %******************************************************************;
		...
		%ELSE %IF &mlen. ^= 0 AND &mflen. = 0 AND &mlblen. ^= 0 %THEN %DO;
          ...
          PUT @3 "SELECT" @12 variable @%EVAL(14+&mvlen.) "LENGTH = " 
				...
				END;
		    ELSE IF STRIP(len) = "" AND STRIP(label) ^= "" THEN DO;
          PUT @3 "SELECT" @12 variable @%EVAL(25+&mvlen.+&mlen.) "LABEL = "
          ...
          PUT @9 "," @12 variable; @%EVAL(14+&mvlen.) "LENGTH = " 
				...
				END;
		    ELSE IF STRIP(len) = "" AND STRIP(label) ^= "" THEN DO;
          PUT @9 "," @12 variable; @%EVAL(25+&mvlen.+&mlen.) "LABEL = "
                                   @%EVAL(33+&mvlen.+&mlen.) label;
  ...
  %******************************************************************;
		...
		%ELSE %IF &mlen. ^= 0 AND &mflen. ^= 0 AND &mlblen. ^= 0 %THEN %DO;
          ...
          PUT @3 "SELECT" @12 variable;
				END;
		    ELSE IF STRIP(len) ^= "" AND STRIP(fmt) = "" AND STRIP(label) = "" THEN DO;
          PUT @3 "SELECT" @12 variable @%EVAL(14+&mvlen.) "LENGTH = " 
                                       @%EVAL(23+&mvlen.) len ;

				END;
		    ELSE IF STRIP(len) = "" AND STRIP(fmt) ^= "" AND STRIP(label) = "" THEN DO;
          PUT @3 "SELECT" @12 variable @%EVAL(26+&mvlen.+&mlen.) "FORMAT = "
                                       @%EVAL(35+&mvlen.+&mlen.) fmt;

				END;
		    ELSE IF STRIP(len) = "" AND STRIP(fmt) = "" AND STRIP(label) ^= "" THEN DO;
          PUT @3 "SELECT" @12 variable @%EVAL(36+&mvlen.+&mlen.+&mflen.) "LABEL = "
          ...
          PUT @9 "," @12 variable @%EVAL(14+&mvlen.) "LENGTH = " 
				...
				END;
		    ELSE IF STRIP(len) = "" AND STRIP(fmt) ^= "" AND STRIP(label) = "" THEN DO;
          PUT @9 "," @12 variable @%EVAL(26+&mvlen.+&mlen.) "FORMAT = "
                                  @%EVAL(35+&mvlen.+&mlen.) fmt;

				END;
		    ELSE IF STRIP(len) = "" AND STRIP(fmt) = "" AND STRIP(label) ^= "" THEN DO;
          PUT @9 "," @12 variable @%EVAL(36+&mvlen.+&mlen.+&mflen.) "LABEL = "
																	@%EVAL(44+&mvlen.+&mlen.+&mflen.) label;
...
%c_attrib(dataset    = adsl , 
          path       = \\server\Projects\Agilis-SA\21LAQ01\ADaMF\specification, 
          xlsx_file  = ADAM define-21LAQ01_v2.xlsx  ,
          from       = pre_adae);
