...
%MACRO dtc2dtm( dtc=, da=, dafmt=, tm=, tmfmt=, dtm=, dtmfmt=);
  ...
  IF STRIP(&dtc.) ^= "" THEN DO;
    ...
    %IF %LENGTH(&da.) %THEN %DO;
      &da.=INPUT( SUBSTR(STRIP(&dtc.), 1, 10) , %IF %LENGTH(&dafmt.) %THEN
        %UPCASE(&dafmt.);
      %ELSE IS8601DA. ;
      );
    %END;
    ...
    %IF %LENGTH(&tm.) %THEN %DO;
      IF LENGTH(STRIP(&dtc.)) > 10 THEN DO;
        &tm.=INPUT( SUBSTR(STRIP(&dtc.), 12) , %IF %LENGTH(&tmfmt.) %THEN
          %UPCASE(&tmfmt.);
        %ELSE TIME8. ;
        );
      END;
    ...
    %IF %LENGTH(&dtm.) ^= 0 %THEN %DO;
      IF LENGTH(STRIP(&dtc.)) > 10 THEN DO;
        &dtm.=INPUT(STRIP(&dtc.), %IF %LENGTH(&dtmfmt.) %THEN %UPCASE(&dtmfmt.);
        %ELSE IS8601DT. ;
        );
      END;
    %END;
  ...
  FORMAT %IF %LENGTH(&da.) AND %LENGTH(&dafmt.) %THEN &da. %UPCASE(&dafmt.) ;
  %ELSE %IF %LENGTH(&da.) AND %LENGTH(&dafmt.)=0 %THEN &da. IS8601DA.;
  %IF %LENGTH(&tm.) AND %LENGTH(&tmfmt.) %THEN &tm. %UPCASE(&tmfmt.) ;
  %ELSE %IF %LENGTH(&tm.) AND %LENGTH(&tmfmt.)=0 %THEN &tm. TIME8. ;
  %IF %LENGTH(&dtm.) AND %LENGTH(&dtmfmt.) %THEN &dtm. %UPCASE(&dtmfmt.);
  %ELSE %IF %LENGTH(&dtm.) AND %LENGTH(&dtmfmt.)=0 %THEN &dtm. IS8601DT.;
  ;
...
%MACRO select_rand_subj(din=, vname=, select=) / MINOPERATOR MINDELIMITER='#';
  ...
  %IF NOT %SYSFUNC(EXIST(&din.)) %THEN %DO;
    %PUT %SYSFUNC(CATX(%STR(), WAR, %STR(NING:)))%STR( Dataset )%UPCASE(&din.)
      does not exist.;
    %PUT %SYSFUNC(CATX(%STR(), ERR, %STR(OR:)))%STR( SELECT_RAND_SUBJ )macro has
      stopped working.;
    %GOTO EXIT;
  %END;
  ...
  %*B);
  PROC SQL NOPRINT;
    SELECT DISTINCT %IF &vname_typ.=C %THEN %DO;
    QUOTE(COMPRESS(&vname.)) %END;
    %ELSE %DO;
    &vname. %END;
INTO:
    _ptno SEPARATED BY ' ' FROM &din.;
  QUIT;
  ...
  %IF &count. < &select. %THEN %DO;
    %PUT %SYSFUNC(CATX(%STR(), WAR, %STR(NING:)))%STR( Your )total selected
      subject number exceeds the total distinct subjects in %UPCASE(&vname.).;
    %PUT %SYSFUNC(CATX(%STR(), ERR, %STR(OR:)))%STR( SELECT_RAND_SUBJ )macro has
      stopped working.;
    %GOTO EXIT;
  %END;
...
%MACRO dup_check( indata=, byvar=, dup_only=);
  ...
  %IF %LENGTH(&indata.) EQ 0 %THEN %DO;
    ...
    DATA _NULL_;
      PUT "WAR"
        "NING: (DUP_CHECK macro)==> Please insert a valid dataset name in the parameter INDATA.";
  ...
  DATA dupchek_data_1;
    ...
    IF ( FIRST.&last_keyvar. ^= LAST.&last_keyvar. ) OR ( FIRST.&last_keyvar.=
      LAST.&last_keyvar.=0 ) THEN DO;
      duplicate="YES";
      dup_flag=1;
    END;
    ...
    IF eof AND dup_flag=0 THEN DO;
      ...
      CALL SYMPUTX ("dup_comment" ,
        "==> No duplicate records found for the key variables in dataset %UPCASE(&indata.)");
    END;
    ELSE IF eof AND dup_flag=1 THEN DO;
      CALL SYMPUTX ("dup_flag" , dup_flag);
      CALL SYMPUTX ("dup_comment" ,
        "==> Duplicate records found for the key variables in dataset %UPCASE(&indata.). See the result on output.");
    END;
  RUN;
  ...
  %IF &dup_flag.=1 %THEN %DO;
    ...
    PROC FREQ DATA=dupchek_data_1;
      ...
      %IF %UPCASE(&dup_only.)=YES OR %UPCASE(&dup_only.)=Y %THEN WHERE duplicate
        ="YES";
      ;
    RUN;
  %END;
  ...
  PROC DATASETS LIB=WORK MEMTYPE=DATA NOLIST;
    DELETE dupchek_data_srt dupchek_data_1;
  QUIT;
  RUN;
...
%MACRO change_check( indata=, byvar=);
  ...
  %IF %LENGTH(&indata.) EQ 0 %THEN %DO;
    ...
    DATA _NULL_;
      PUT "WAR"
        "NING: (CHANGE_CHECK macro)==> Please insert a valid dataset name in the parameter INDATA.";
  ...
  *Modify;
  ...
  DATA chng_check_show ( KEEP=&byvar. change_in_&last_keyvar. );
    ...
    %*Change;
    IF ( FIRST.&base_keyvar. ^= LAST.&base_keyvar. ) OR ( FIRST.&base_keyvar.=
      LAST.&base_keyvar.=0 ) THEN DO;
    ...
    IF eof and is_change=1 THEN DO;
      ...
      CALL SYMPUTX ("chng_comment" ,
        "==> Changes found in the variable %UPCASE(&last_keyvar.) for other key variables in the dataset %UPCASE(&indata.)");

    END;
    ELSE IF eof and is_change=0 THEN DO;

      change_in_&last_keyvar.=
        "No changes found in %UPCASE(&last_keyvar.) for other key variables.";
      CALL SYMPUTX ("is_change" , is_change );
      CALL SYMPUTX ("chng_comment" ,
        "==> No changes found in the variable %UPCASE(&last_keyvar.) for other key variables in the dataset %UPCASE(&indata.)");
      OUTPUT;
  ...
  %IF &is_change.=1 %THEN %DO;
    ...
    PROC REPORT DATA=chng_check_show NOWD HEADLINE HEADSKIP SPACING=3 ;
      COLUMN ("Changes of %UPCASE(&last_keyvar.)" &byvar.
        change_in_&last_keyvar. );
    RUN;
    TITLE3;
  ...
  %*Deleting intermediate datasets.;
  PROC DATASETS LIB=WORK MEMTYPE=DATA NOLIST;
    DELETE change_check_srt chng_check_1 chng_check_show;
  QUIT;
  RUN;
...
%MACRO ads_order_check( libname=, qc_data=, main_data=);
  ...
  %******************************************************************;
  PROC SQL NOPRINT;
    SELECT STRIP(name) INTO : qc_vlist SEPARATED BY "@@@" FROM sashelp.vcolumn
      WHERE UPCASE(libname)=UPCASE("&libname.") AND UPCASE(memname)=
      UPCASE("&qc_data.");

    SELECT STRIP(name) INTO : main_vlist SEPARATED BY "@@@" FROM sashelp.vcolumn
      WHERE UPCASE(libname)=UPCASE("&libname.") AND UPCASE(memname)=
      UPCASE("&main_data.");
  QUIT;
  ...
  %******************************************************************;
  %IF %SYSFUNC( COUNTW(&qc_vlist., %STR(@@@)) ) ^= %SYSFUNC(
    COUNTW(&main_vlist., %STR(@@@)) ) %THEN %DO;

    DATA _NULL_;
      PUT "WAR"
        "NING: Number of variables are not same in the QC and main data.";
  ...
  DATA _NULL_;
    ...
    %DO i=1 %TO %SYSFUNC(COUNTW(&qc_vlist., %STR(@@@)));
      ...
      %IF &qc_var.=&main_var. %THEN %DO;
        PUT "Order = &i." @14"QC variable = &qc_var."
          @45"| Main variable = &main_var."
          @80"| Variable is in order with QC in ADS";
      %END;
      %ELSE %DO;
        PUT "Order = &i." @14"QC variable = &qc_var."
          @45"| Main variable = &main_var."
          @80"| Variable is not in order or case issue in ADS (Caution)";
      %END;
...
%MACRO basic_log_checker( log_file=, programmer=);
  ...
  DATA _NULL_;
    ...
    ELSE IF INDEX(UPCASE(line), ( "UNIN" || "ITIA" || "LIZED" ) ) THEN DO;
      un_inti + 1;
    END;
    ELSE IF INDEX(UPCASE(COMPBL(line)), ( "AT LEAST ONE W" ||
      ".D FORMAT WAS TOO SMALL") ) THEN DO;
      wd + 1;
    END;
    ELSE IF INDEX(UPCASE(line), ( "REP" || "EATS OF B" || "Y VALUES" ) ) THEN
      DO;
      by_repeat + 1;
    END;
    ...
    ELSE IF FIND(UPCASE(COMPRESS(line)), ("MSG"||"LEVEL=I")) THEN DO;
      msg_level + 1;
    END;
    ELSE IF INDEX(UPCASE(COMPBL(line)), ( "NUM" || "ERIC VALUES HAVE BE" ||
      "EN CONVE" || "RTED TO CHA" || "RACTER VALUES AT THE PLA" ||
      "CES GIVEN BY" ) ) THEN DO;
      num_to_char + 1;
    END;
    ELSE IF INDEX(UPCASE(COMPBL(line)), ( "CHA" || "RACTER VALUES HAVE BE" ||
      "EN CONVE" || "RTED TO NUM" || "ERIC VALUES AT THE PLA" || "CES GIVEN BY"
      ) ) THEN DO;
      char_to_nam + 1;
    END;
    ELSE IF INDEX(UPCASE(COMPBL(line)), ( "MIS" || "SING VALUES WE" || "RE GENE"
      || "RATED" ) ) OR INDEX(UPCASE(COMPBL(line)), ( "PERFO" ||
      " RMING AN OPER" || "ATION ON MIS" || "SING VALUES" ) ) THEN DO;
      generate_missing + 1;
    END;
    ELSE IF ( ( UPCASE(COMPRESS(SUBSTR(line,1,6))) ^= "ERR" || "OR:") AND
      (UPCASE(COMPRESS(SUBSTR(line,1,8))) ^= "WAR" || "NING:" ) ) AND (
      INDEX(UPCASE(COMPBL(line)), "INV" || "ALID DATA FOR") OR
      INDEX(UPCASE(COMPRESS(line)), "---+----1----+---") ) THEN DO;
      in_valid + 1;
    END;
  ...
  DATA _NULL_;
    ...
    PUT "* NUMBER OF --> GENE" "RATE MIS" "SING VALUES ISSUE"
      @59"= &generate_missing.";
    ...
    PUT "* NUMBER OF --> NUM" "MERIC VARIABLE HAVE BEEN CONV" "ERTED..."
      @59"= &num_to_char.";
    PUT "* NUMBER OF --> CHAR" "ACTER VARIABLE HAVE BEEN CONV" "ERTED..."
      @59"= &char_to_nam.";
    PUT "* NUMBER OF --> INV" "ALID DATA ISSUE" @59"= &in_valid.";
    PUT "*";
    %IF &msg_level.=0 %THEN PUT "* NOTE: 'OPTION MSG" "LEVEL = I' IS NOT FO"
      "UND.";
    ;
    PUT "*";
    PUT "*";
...
* Version - 2;
%MACRO basic_log_checker( log_file=, programmer=, summary_inlog=) / MINOPERATOR
  MINDELIMITER='#';
  ...
  DATA log_data ;
    ...
    ELSE IF INDEX(UPCASE(COMPBL(line)), ( "MIS" || "SING VALUES WE" || "RE GENE"
      ...
      OUTPUT;
    END;
    ELSE IF INDEX(UPCASE(COMPBL(line)), ( "DU" || "PLI" || "CATE BY VARI" ||
      "ABLE(S) SPE" || "CIFIED" ) ) THEN DO;
      dup_by_var + 1;
      ...
      OUTPUT;
    END;
    ELSE IF INDEX(UPCASE(COMPBL(line)), ( "DIVI" || "SION BY ZE" || "RO DET" ||
      "ECTED") ) THEN DO;
      div_by_zero + 1;
      ...
      OUTPUT;
    END;
    ELSE IF INDEX(UPCASE(COMPBL(line)), ( "MATHEM" || "ATICAL OPE" ||
      "RATIONS COULD N" || "OT BE PER" || "FORMED" ) ) THEN DO;
      no_math_op + 1;
      message="MATHEM" || "ATICAL OPE" || "RATIONS COULD N" || "OT BE PER" ||
        "FORMED" ;
      serial=serial + 1;
      alerts_fl=1;
      OUTPUT;
    ...
    ELSE IF ( ( UPCASE(COMPRESS(SUBSTR(line,1,6))) ^= "ERR" || "OR:") AND
      (UPCASE(COMPRESS(SUBSTR(line,1,8))) ^= "WAR" || "NING:" ) ) AND (
      INDEX(UPCASE(COMPBL(line)), "INV" || "ALID DATA FOR") OR
      INDEX(UPCASE(COMPRESS(line)), ( "--" || "-+" || "---" || "-1" || "---" ||
      "-+" || "--" ) ) ) THEN DO;
      in_valid + 1;
      message="INV" || "ALID DATA OR " || "--" || "-+" || "---" || "-1" || "---"
        || "-+" || "--- ISSUE";
      serial=serial + 1;
      alerts_fl=1;
      OUTPUT;
    ...
    ELSE IF INDEX(UPCASE(COMPBL(line)), ( "INV" || "ALID" ) ) THEN DO;
      ...
      OUTPUT;
    END;
    ELSE IF INDEX(UPCASE(COMPBL(line)), ( "UNA" || "BLE TO" || " OPEN" ) ) OR
      INDEX(UPCASE(COMPBL(line)), ( "WILL BE OPE" || "NED INS" || "TEAD" ) )
      THEN DO;
      una_ble + 1;
      ...
      OUTPUT;
    END;
    ELSE IF INDEX(UPCASE(COMPBL(line)), ( "ERR" || "OR" ) ) OR
      INDEX(UPCASE(COMPBL(line)), ( "WAR" || "NING" ) ) THEN DO;
      pgm_err + 1;
  ...
  PROC REPORT DATA=log_data NOWD HEADLINE HEADSKIP SPACING=3 ;
    COLUMN ("GENERAL SAS ALERTS LIST IN %UPCASE(&programmer.) - LOG FILE" " "
      "LOG: &log_file." " " serial message line);
    DEFINE serial / "SL. No" ;
  ...
  DATA log_summary;
    ...
    message="ERR"
      ||"ORS | (Check the list to be ensure that they are not programmer's written)";
    freq=&err.;
    serial=1;
    OUTPUT;
    message="WAR" ||
      "NINGS | (Check the list to be ensure that they are not programmer's written)";
    freq=&warn.;
    serial=2;
    OUTPUT;
  ...
  PROC REPORT DATA=log_summary NOWD HEADLINE HEADSKIP SPACING=3 ;
    COLUMN
      ("SUMMARY OF GENERAL SAS ALERTS LIST IN %UPCASE(&programmer.) - LOG FILE"
      " " "LOG: &log_file." "" serial message freq);
    DEFINE serial / "SL. No" ;
  ...
  %IF %UPCASE(&summary_inlog.) IN(y#yes#Y#YES) %THEN %DO;
    ...
    DATA _NULL_;
      ...
      PUT "* NUMBER OF --> GENE" "RATE MIS" "SING VALUES ISSUE"
        @59"= &generate_missing.";
      PUT "* NUMBER OF --> DU" "PLI" "CATE BY VARI" "ABLE(S) SPE" "CIFIED"
        @59"= &dup_by_var.";
      PUT "* NUMBER OF --> DIVI" "SION BY ZE" "RO DET" "ECTED"
        @59"= &div_by_zero.";
      PUT "* NUMBER OF --> MATHEM" "ATICAL OPE" "RATIONS N" "OT PER"
        "FORMED"@59"= &div_by_zero.";
      PUT "* NUMBER OF --> W" ".D FORMAT ISSUE" @59"= &wd.";
      PUT "* NUMBER OF --> RE" "PEAT OF BY VAL" "UES ISSUE" @59"= &by_repeat.";
      PUT "* NUMBER OF --> VARIABLE OVE" "RWRITTEN ISSUE" @59"= &by_overwrite.";
      PUT "* NUMBER OF --> NUM" "MERIC VARIABLE HAVE BEEN CONV" "ERTED..."
      ...
      PUT "* NUMBER OF --> CHAR" "ACTER VARIABLE HAVE BEEN CONV" "ERTED..."
        @59"= &char_to_nam.";
      PUT "* NUMBER OF --> INV" "ALID DATA ISSUE" @59"= &in_valid.";
      PUT "* NUMBER OF --> SOMETHING REGARDING INV" "ALID STATMENTS"
        @59"= &inv_alid_stat.";
      PUT "* NUMBER OF --> PROGRAMMER'S WRITTEN ER" "ROR OR WAR" "NING"
        @59"= &pgm_err.";
